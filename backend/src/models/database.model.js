...
class DatabaseModel {
  ...
  // insertUnique
  async insertUnique(db, tableName, data, uniqueField) {
    try {
      return await new Promise((resolve, reject) => {
        const checkQuery = `SELECT COUNT(*) as count FROM ${tableName} WHERE ${uniqueField} = ?`;
        db.query(checkQuery, [data[uniqueField]], (err, countRes) => {
          if (err) {
            console.log(err);
            return reject({ msg: ERROR });
          }
          ...
          if (countRes[0].count > 0) {
            return reject({ msg: `${uniqueField} đã tồn tại` });
          }
          const query = `INSERT INTO ${tableName} SET ?`;
          db.query(query, data, (err, dataRes) => {
            if (err) {
